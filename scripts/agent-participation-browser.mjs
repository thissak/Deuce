...
try {
  ...
  await page.getByRole('button', { name: 'AI 관리', exact: true }).waitFor()
  await page.getByRole('button', { name: '설정', exact: true }).click()
  await page.getByLabel('AI 이름', { exact: true }).fill('테스트 Codex')
  await page.getByRole('radio', { name: '채널만 참여', exact: true }).check()
  ...
  await page.getByText('기존 MCP 방식으로 연결', { exact: true }).click()
  ...
  await page.getByRole('button', { name: 'AI 관리', exact: true }).click()
