...
서버 #14·PC 실행기 #15·채팅 UI #16을 리뷰 후 머지하고 운영 서버·0.2.0 앱을 배포했다.
새 서명 앱과 운영의 검증 결과는 [배포 이관](2026-09-08-ai-invite-release.md)에 기록했다.
...
| 검증 | 결과 |
|---|---|
| 전체 자동 테스트 | 270 통과: 서버 94·웹 156·데스크톱 12·실행기 6·MCP 2 |
...
| PC 프로세스 | Execa의 Windows CLI/.cmd·공백 경로·빈 인자·stdin 보존 및 취소 시 자식 종료, Windows CI 통과 |
| 데스크톱 컨트롤러 | 암호화만 저장·계정별 복구·계정 변경 중단·암호화 불가 시 세션만·로그아웃 중 연결 완료 거부·오래된 복구 응답 차단 |
...
리뷰·운영 백업/migration·서버/웹·앱 배포와 실제 Mac 검증은
[배포 이관](2026-09-08-ai-invite-release.md)에서 완료했다.

1. Windows/Intel Mac CLI 및 패키지 실기 검증. 브라우저 반응형 검증은 iOS/Android 실기를 의미하지 않는다.
2. AI 연결을 저장한 구버전에서 다음 버전으로 업데이트했을 때의 복구 검증.
3. 다른 사용자 3~5명에게 최초 CLI 설치/로그인·AI 연결·한 번 초대 과제를 수행하게 하고 어려운 지점을 조사.
