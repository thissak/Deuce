...
첫 버티컬 슬라이스의 Flutter 클라이언트와 TypeScript 서버가 공유하는 개발 계약이다.
현재 단일 채널과 개발 fixture 사용자만 지원한다.
...
- `authorId`는 현재 `alice` 또는 `bob`만 허용한다.
...
이는 인증이 아닌 개발 fixture다. 지원하지 않는 값은 `invalid_dev_user` 연결 오류로
거절한다.
...
실패 acknowledgement는 `{"ok": false, "error": "invalid_message"}` 또는
`store_failed`다. 서버는 DB 저장에 성공한 뒤에만 `message:created` 이벤트로 공통
메시지를 채널에 방송한다. 동일 작성자가 같은 `clientMessageId`를 재전송하면 기존
메시지를 acknowledgement로 돌려주고 다시 저장하거나 방송하지 않는다.
