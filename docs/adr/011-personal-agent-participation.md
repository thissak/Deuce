...
2026-09-11: [ADR 013](013-retire-ai-and-replan.md)으로 대체했다. 아래는 이전 구현의 결정 기록이며 제거 변경은 미배포다.

Accepted (2026-09-08 감독 승인, #8). 로컬 구현·검증이며 운영 적용 전이다.
ADR 007의 신규 등록 모델·연결 관리 권한과 ADR 008의 도구 목록을 확장한다.

후속 [ADR 012](012-invite-and-run-personal-ai.md)가 신규 등록 기본값을 SELECTED로 변경하고
요청 시 PC 실행기와 실행 중 취소를 추가한다. 아래 내용은 기존 연결의 호환 계약이다.
